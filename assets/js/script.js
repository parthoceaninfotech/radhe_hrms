...
(function () {
	...
	function positionMiniSubmenu($link) {
		if (!$('body').hasClass('mini-sidebar')) return;
		var $ul = $link.next('ul');
		if (!$ul.length) return;
		var rect = $link[0].getBoundingClientRect();
		var top = Math.max(10, Math.min(rect.top, window.innerHeight - $ul.outerHeight() - 10));
		$ul.css({ top: top + 'px' });
	}
	...
	function init() {
		...
		$('.sidebar-menu a').on('click', function(e) {
			...
			var $submenuUl = $(this).next('ul');
			if(!$(this).hasClass('subdrop')) {
				$('ul', $(this).parents('ul:first')).each(function() {
					if (isMini) $(this).hide();
					else $(this).slideUp(250);
				});
				$('a', $(this).parents('ul:first')).removeClass('subdrop');
				if (isMini && $submenuUl.length) {
					$submenuUl.show();
					positionMiniSubmenu($(this));
				} else {
				...
				if (isMini) $submenuUl.hide();
				else $submenuUl.slideUp(350);
			}
		});
	...
	$(document).on('click', function(e) {
		if (!$('body').hasClass('mini-sidebar')) return;
		if (!$(e.target).closest('.sidebar-menu .submenu').length) {
			$('.sidebar-menu .subdrop').removeClass('subdrop');
			$('.sidebar-menu > ul > li.submenu > ul').hide();
		}
	});

	$(window).on('resize scroll', function() {
		var $open = $('.sidebar-menu > ul > li.submenu > a.subdrop');
		if ($open.length) positionMiniSubmenu($open.first());
	});
	...
	$(document).on('click', '#toggle_btn', function () {
		...
		if (isMini) {
			...
			$('.sidebar-menu > ul > li.submenu > ul').removeAttr('style');
			...
			$body.addClass('mini-sidebar').removeClass('expand-menu');
			$('.sidebar-menu .subdrop').removeClass('subdrop');
			$('.sidebar-menu > ul > li.submenu > ul').hide().removeAttr('style');
