...
if ($action === 'view_rates') {
    $rates = $ai_db->aiGetQuery("SELECT * FROM hrms_gratuity_rates WHERE company_id = $company_id ORDER BY effective_date DESC, id DESC");
    echo json_encode([
        'status' => 'success',
        'data' => $rates
    ]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $gratuity_rate = floatval($_POST['gratuity_rate'] ?? 0);
        ...
        if ($id > 0) {
            $sql = "UPDATE hrms_gratuity_rates SET 
                        gratuity_rate = $gratuity_rate,
                        effective_date = '$effective_date',
                        updated_by = '$username'
                    WHERE id = $id AND company_id = $company_id";
        } else {
            $sql = "INSERT INTO hrms_gratuity_rates (
                        company_id, gratuity_rate, effective_date, created_by, updated_by
                    ) VALUES (
                        $company_id, $gratuity_rate, '$effective_date', '$username', '$username'
                    )";
        }

        if ($ai_db->aiQuery($sql)) {
            $rate_id = ($id > 0) ? $id : $ai_db->aiLastInsert();
            echo json_encode(['status' => 'success', 'message' => 'Gratuity Rate saved successfully.', 'rate_id' => $rate_id]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to save Gratuity Rate.']);
        }
    } else {
    ...
    if ($id > 0) {
        $result = $ai_db->aiQuery("DELETE FROM hrms_gratuity_rates WHERE id = $id AND company_id = $company_id");
        if ($result) {
            echo json_encode(['status' => 'success', 'message' => 'Gratuity Rate deleted successfully.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to delete Gratuity Rate.']);
        }
    } else {
