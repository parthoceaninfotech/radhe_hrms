...
if ($action === 'view_rates') {
    $rates = $ai_db->aiGetQuery("SELECT * FROM hrms_glwf_rates WHERE company_id = $company_id ORDER BY state_name ASC, effective_date DESC, id DESC");
    echo json_encode([
        'status' => 'success',
        'data' => $rates
    ]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $glwf_rate = floatval($_POST['glwf_rate'] ?? 0);
        $company_rate = floatval($_POST['company_rate'] ?? 0);
        $deduct_months = mysqli_real_escape_string($ai_conn, $_POST['deduct_months'] ?? '');
        ...
        if ($id > 0) {
            $sql = "UPDATE hrms_glwf_rates SET 
                        state_name = '$state_name',
                        glwf_rate = $glwf_rate,
                        company_rate = $company_rate,
                        effective_date = '$effective_date',
                        deduct_months = '$deduct_months',
                        updated_by = '$username'
                    WHERE id = $id AND company_id = $company_id";
        } else {
            $sql = "INSERT INTO hrms_glwf_rates (
                        company_id, state_name, glwf_rate, company_rate, 
                        effective_date, deduct_months, created_by, updated_by
                    ) VALUES (
                        $company_id, '$state_name', $glwf_rate, $company_rate, 
                        '$effective_date', '$deduct_months', '$username', '$username'
                    )";
        }

        if ($ai_db->aiQuery($sql)) {
            $rate_id = ($id > 0) ? $id : $ai_db->aiLastInsert();
            echo json_encode(['status' => 'success', 'message' => 'GLWF Rate saved successfully.', 'rate_id' => $rate_id]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to save GLWF Rate.']);
        }
    } else {
    ...
    if ($id > 0) {
        $result = $ai_db->aiQuery("DELETE FROM hrms_glwf_rates WHERE id = $id AND company_id = $company_id");
        if ($result) {
            echo json_encode(['status' => 'success', 'message' => 'GLWF Rate deleted successfully.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to delete GLWF Rate.']);
        }
    } else {
