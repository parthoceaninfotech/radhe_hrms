...
// Auto-migrate tbl_settings columns if missing
$_settings_cols = $ai_db->aiGetQuery("SHOW COLUMNS FROM tbl_settings LIKE 'header_btn_name'");
if (empty($_settings_cols)) {
	$ai_db->aiQuery("ALTER TABLE `tbl_settings` ADD `header_btn_name` VARCHAR(255) DEFAULT 'Get Free Demo'");
	$ai_db->aiQuery("ALTER TABLE `tbl_settings` ADD `header_btn_link` VARCHAR(255) DEFAULT 'https://ocean-crm.oceaninfotechcrm.com/software/register'");
}
?>