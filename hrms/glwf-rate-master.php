<?php
$pageTitle = "GLWF Rate Master - Payroll System";
include 'header.php';
?>
  ...
  <div id="draggableCard" class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-settings me-2" style="font-size: 16px;"></i>GLWF RATE MASTER
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      <form id="glwfForm">
        <input type="hidden" name="id" id="glwf_rate_id" value="0">

        <!-- Top Fields: State, Rates, Effective Date -->
        <fieldset class="border p-3 rounded mb-3" style="border-color: #135ca3 !important; background-color: #ffffff;">
          ...
          <div class="row g-2 align-items-center">
            ...
            <div class="col-md-6 d-flex align-items-center">
              <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 90px;">GLWF
                Rate</label>
              <input type="number" step="0.001" class="form-control form-control-sm text-end" name="glwf_rate"
                id="glwf_rate" value="0.000" required style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
            </div>
            ...
            <div class="col-md-6 d-flex align-items-center">
              <label class="fw-semibold text-dark-blue me-2 text-start"
                style="font-size: 11px; min-width: 90px;">Company Rate</label>
              <input type="number" step="0.001" class="form-control form-control-sm text-end" name="company_rate"
                id="company_rate" value="0.000" required
                style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
            </div>
        ...
        <!-- Month Checkboxes for LWF Deduction -->
              ...
              echo '
                <div class="col-md-3 col-6">
                  <div class="form-check">
                    <input class="form-check-input border-secondary month-chk" type="checkbox" value="' . $m . '" id="month_' . $idx . '" />
                    <label class="form-check-label fw-semibold text-dark" for="month_' . $idx . '" style="font-size: 11px;">' . $m . '</label>
                  </div>
                </div>';
            }
            ?>
    ...
    <div class="modal-content border shadow-lg"
      ...
      <div class="modal-header text-white p-2 px-3"
        ...
        <h6 class="modal-title fw-bold text-white d-flex align-items-center" id="rateSelectModalLabel"
          ...
          <i class="ti ti-search me-2" style="font-size: 15px;"></i>Select GLWF Rate Record
        </h6>
          ...
          <table class="table table-sm table-striped table-bordered table-hover mb-0" style="font-size: 11px;">
            <thead class="table-light">
              <tr>
                ...
                <th class="text-end">GLWF Rate</th>
                <th class="text-end">Company Rate</th>
              </tr>
            </thead>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const form = document.getElementById("glwfForm");
    const monthChks = document.querySelectorAll(".month-chk");
    ...
    function clearForm() {
      form.reset();
      document.getElementById("glwf_rate_id").value = "0";
      ...
      monthChks.forEach(chk => chk.checked = false);
    }
    ...
    function displayRecord(index) {
      ...
      document.getElementById("glwf_rate_id").value = rec.id;
      document.getElementById("recordIndicator").textContent = rec.id;
      document.getElementById("state_name").value = rec.state_name;
      document.getElementById("glwf_rate").value = parseFloat(rec.glwf_rate).toFixed(3);
      document.getElementById("company_rate").value = parseFloat(rec.company_rate).toFixed(3);
      document.getElementById("effective_date").value = formatDateForDisplay(rec.effective_date);

      // Check deduct months checkboxes
      if (rec.deduct_months) {
        const activeMonths = rec.deduct_months.split(',');
        monthChks.forEach(chk => {
          if (activeMonths.includes(chk.value)) {
            chk.checked = true;
          }
        });
      }
    ...
    function fetchRates(openSearch = false) {
      fetch('actions/glwf-rate-master-action.php?action=view_rates')
        .then(res => res.json())
        .then(response => {
    ...
    function populateSearchModal() {
      ...
      ratesList.forEach((rec, index) => {
        ...
        row.innerHTML = `
          ...
          <td class="text-end">${parseFloat(rec.glwf_rate).toFixed(3)}</td>
          <td class="text-end">${parseFloat(rec.company_rate).toFixed(3)}</td>
        `;
    ...
    document.getElementById("btnDelete").addEventListener('click', () => {
      const rateId = document.getElementById("glwf_rate_id").value;
      if (rateId <= 0) return;
      if (!confirm("Are you sure you want to delete this GLWF configuration?")) return;

      fetch(`actions/glwf-rate-master-action.php?action=delete_rate&id=${rateId}`)
        .then(res => res.json())
        .then(response => {
    ...
    document.getElementById("btnSave").addEventListener('click', () => {
      ...
      monthChks.forEach(chk => {
        if (chk.checked) {
          activeMonths.push(chk.value);
        }
      });

      const formData = new FormData();
      formData.append('id', document.getElementById("glwf_rate_id").value);
      formData.append('state_name', stateName);
      formData.append('glwf_rate', document.getElementById("glwf_rate").value);
      formData.append('company_rate', document.getElementById("company_rate").value);
      formData.append('effective_date', effectiveDate);
      formData.append('deduct_months', activeMonths.join(','));

      fetch('actions/glwf-rate-master-action.php?action=save_rate', {
        method: 'POST',
        body: formData
      })
        .then(res => res.json())
