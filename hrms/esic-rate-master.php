<?php
$pageTitle = "ESIC Rate Master - Payroll System";
include 'header.php';
?>
  ...
  <div id="draggableCard" class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-settings me-2" style="font-size: 16px;"></i>ESIC RATE MASTER
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      <form id="esicForm">
        <input type="hidden" name="id" id="esic_rate_id" value="0">

        <!-- Top Fields: Rates, Date -->
        <fieldset class="border p-3 rounded mb-3" style="border-color: #135ca3 !important; background-color: #ffffff;">
          ...
          <div class="row g-2 align-items-center">
            ...
            <div class="col-md-8 d-flex align-items-center">
              <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 120px;">Employee Cont.</label>
              <input type="number" step="0.001" class="form-control form-control-sm text-end" name="employee_rate" id="employee_rate" value="0.750" required style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
            </div>
            ...
            <div class="col-md-8 d-flex align-items-center">
              <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 120px;">Employer Cont.</label>
              <input type="number" step="0.001" class="form-control form-control-sm text-end" name="employer_rate" id="employer_rate" value="3.250" required style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
            </div>
            ...
            <!-- Effective Date -->
            <div class="col-md-8 d-flex align-items-center">
              <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 120px;">Effective Date</label>
    ...
    <div class="modal-content border shadow-lg" style="border-radius: 6px !important; border-color: #a3b8cc !important;">
      <div class="modal-header text-white p-2 px-3" style="background: linear-gradient(90deg, #135ca3 0%, #00a2e8 100%); border-top-left-radius: 5px !important; border-top-right-radius: 5px !important; border-bottom: 1px solid #104f9b;">
        <h6 class="modal-title fw-bold text-white d-flex align-items-center" id="rateSelectModalLabel" style="font-size: 13px; margin: 0;">
          <i class="ti ti-search me-2" style="font-size: 15px;"></i>Select ESIC Rate Record
        </h6>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function clearForm() {
      form.reset();
      document.getElementById("esic_rate_id").value = "0";
    ...
    function displayRecord(index) {
      ...
      document.getElementById("esic_rate_id").value = rec.id;
      document.getElementById("recordIndicator").textContent = rec.id;
      document.getElementById("employee_rate").value = parseFloat(rec.employee_rate).toFixed(3);
      document.getElementById("employer_rate").value = parseFloat(rec.employer_rate).toFixed(3);
    ...
    function fetchRates(openSearch = false) {
      fetch('actions/esic-rate-master-action.php?action=view_rates')
        .then(res => res.json())
        .then(response => {
    ...
    function populateSearchModal() {
      ...
      ratesList.forEach((rec, index) => {
        ...
        row.innerHTML = `
          ...
          <td class="text-end">${parseFloat(rec.employee_rate).toFixed(3)}</td>
          <td class="text-end">${parseFloat(rec.employer_rate).toFixed(3)}</td>
        `;
    ...
    document.getElementById("btnDelete").addEventListener('click', () => {
      const rateId = document.getElementById("esic_rate_id").value;
      if (rateId <= 0) return;
      if (!confirm("Are you sure you want to delete this ESIC rate?")) return;

      fetch(`actions/esic-rate-master-action.php?action=delete_rate&id=${rateId}`)
        .then(res => res.json())
        .then(response => {
    ...
    document.getElementById("btnSave").addEventListener('click', () => {
      ...
      formData.append('id', document.getElementById("esic_rate_id").value);
      formData.append('employee_rate', document.getElementById("employee_rate").value);
      formData.append('employer_rate', document.getElementById("employer_rate").value);
      formData.append('effective_date', effectiveDate);

      fetch('actions/esic-rate-master-action.php?action=save_rate', {
        method: 'POST',
        body: formData
      })
      .then(res => res.json())
