<?php
$pageTitle = "Salary Component For ESIC Calculation - Payroll System";
include 'header.php';
?>
  ...
  <div id="draggableCard" class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-settings me-2" style="font-size: 16px;"></i>ESIC CALCULATED ON COMPONENTS
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <div id="selectBranchPlaceholder" class="text-center py-4 border rounded" style="background-color: #f8f9fa; border-style: dashed !important; border-color: #cbd5e1 !important;">
        ...
        <p class="text-muted mb-0" style="font-size: 12px;">Please select a Branch to configure ESIC components.</p>
      </div>

      <!-- Main UI (Hidden or disabled until branch is selected) -->
      <div id="componentsContainer" style="display: none;">
        <form id="esicComponentsForm">
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load branches
    fetch('actions/esic-rate-master-action.php?action=get_branches')
      .then(res => res.json())
      .then(response => {
    ...
    function fetchComponents(branchId) {
      ...
      fetch(`actions/esic-rate-master-action.php?action=get_components&branch_id=${branchId}`)
        .then(res => res.json())
        .then(response => {
          if (response.status === 'success') {
    ...
    document.getElementById("esicComponentsForm").addEventListener('submit', (e) => {
      e.preventDefault();
      ...
      fetch('actions/esic-rate-master-action.php?action=save_components', {
        method: 'POST',
        body: formData
      })
      .then(res => res.json())
