<?php
$pageTitle = "Bonus Rate Master - Payroll System";
include 'header.php';
?>
  ...
  <div id="draggableCard" class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-settings me-2" style="font-size: 16px;"></i>BONUS RATE MASTER
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      <form id="bonusForm">
        <input type="hidden" name="id" id="bonus_rate_id" value="0">

        <!-- Top Fields: Rate, Date, Limits -->
        <fieldset class="border p-3 rounded mb-3" style="border-color: #135ca3 !important; background-color: #ffffff;">
          ...
          <div class="row g-2 align-items-center">
            ...
            <!-- Left Side: Bonus Rate and Date -->
            <div class="col-md-6">
              <div class="row g-2 align-items-center">
                <div class="col-12 d-flex align-items-center">
                  <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 90px;">Bonus Rate</label>
                  <input type="number" step="0.001" class="form-control form-control-sm text-end" name="bonus_rate" id="bonus_rate" value="0.000" required style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
                </div>
            ...
            <!-- Right Side: Limit Configurations -->
            <div class="col-md-6 border-start ps-3" style="border-color: #cbd5e1 !important;">
              <div class="row g-2 align-items-center">
                <div class="col-12 d-flex align-items-center">
                  <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 140px;">Pay Basic Limit</label>
                  <input type="number" step="0.01" class="form-control form-control-sm text-end" name="pay_basic_limit" id="pay_basic_limit" value="0.00" style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
                </div>
                <div class="col-12 d-flex align-items-center mt-2">
                  <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 140px;">Bonus Ceiling</label>
                  <input type="number" step="0.01" class="form-control form-control-sm text-end" name="bonus_ceiling" id="bonus_ceiling" value="0.00" style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
                </div>
                <div class="col-12 d-flex align-items-center mt-2">
                  <label class="fw-semibold text-dark-blue me-2 text-start" style="font-size: 11px; min-width: 140px; white-space: nowrap;">Min. Pay days For Bonus</label>
                  <input type="number" class="form-control form-control-sm text-end" name="min_pay_days" id="min_pay_days" value="0" style="font-size: 11px; border: 1px solid #c9c8cc !important;" />
                </div>
              </div>
            </div>

            <!-- Record ID indicator footer -->
    ...
    <div class="modal-content border shadow-lg" style="border-radius: 6px !important; border-color: #a3b8cc !important;">
      <div class="modal-header text-white p-2 px-3" style="background: linear-gradient(90deg, #135ca3 0%, #00a2e8 100%); border-top-left-radius: 5px !important; border-top-right-radius: 5px !important; border-bottom: 1px solid #104f9b;">
        <h6 class="modal-title fw-bold text-white d-flex align-items-center" id="rateSelectModalLabel" style="font-size: 13px; margin: 0;">
          <i class="ti ti-search me-2" style="font-size: 15px;"></i>Select Bonus Rate Record
        </h6>
          ...
          <table class="table table-sm table-striped table-bordered table-hover mb-0" style="font-size: 11px;">
            <thead class="table-light">
              <tr>
                ...
                <th class="text-end">Bonus Rate %</th>
                <th class="text-end">Pay Basic Limit</th>
                <th class="text-end">Bonus Ceiling</th>
                <th class="text-end">Min Pay Days</th>
              </tr>
            </thead>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Format number fields on change
    document.getElementById("bonus_rate").addEventListener('change', function() {
      const val = parseFloat(this.value) || 0;
      this.value = val.toFixed(3);
    });

    document.getElementById("pay_basic_limit").addEventListener('change', function() {
      const val = parseFloat(this.value) || 0;
      this.value = val.toFixed(2);
    });

    document.getElementById("bonus_ceiling").addEventListener('change', function() {
      const val = parseFloat(this.value) || 0;
      this.value = val.toFixed(2);
    });
    ...
    function clearForm() {
      form.reset();
      document.getElementById("bonus_rate_id").value = "0";
    ...
    function displayRecord(index) {
      ...
      document.getElementById("bonus_rate_id").value = rec.id;
      document.getElementById("recordIndicator").textContent = rec.id;
      document.getElementById("bonus_rate").value = parseFloat(rec.bonus_rate).toFixed(3);
      document.getElementById("pay_basic_limit").value = parseFloat(rec.pay_basic_limit).toFixed(2);
      document.getElementById("bonus_ceiling").value = parseFloat(rec.bonus_ceiling).toFixed(2);
      document.getElementById("min_pay_days").value = parseInt(rec.min_pay_days);
    ...
    function fetchRates(openSearch = false) {
      fetch('actions/bonus-rate-master-action.php?action=view_rates')
        .then(res => res.json())
        .then(response => {
    ...
    function populateSearchModal() {
      ...
      ratesList.forEach((rec, index) => {
        ...
        row.innerHTML = `
          ...
          <td class="text-end">${parseFloat(rec.bonus_rate).toFixed(3)}</td>
          <td class="text-end">${parseFloat(rec.pay_basic_limit).toFixed(2)}</td>
          <td class="text-end">${parseFloat(rec.bonus_ceiling).toFixed(2)}</td>
          <td class="text-end">${parseInt(rec.min_pay_days)}</td>
        `;
    ...
    document.getElementById("btnDelete").addEventListener('click', () => {
      const rateId = document.getElementById("bonus_rate_id").value;
      if (rateId <= 0) return;
      if (!confirm("Are you sure you want to delete this Bonus rate configuration?")) return;

      fetch(`actions/bonus-rate-master-action.php?action=delete_rate&id=${rateId}`)
        .then(res => res.json())
        .then(response => {
    ...
    document.getElementById("btnSave").addEventListener('click', () => {
      ...
      formData.append('id', document.getElementById("bonus_rate_id").value);
      formData.append('bonus_rate', document.getElementById("bonus_rate").value);
      formData.append('pay_basic_limit', document.getElementById("pay_basic_limit").value);
      formData.append('bonus_ceiling', document.getElementById("bonus_ceiling").value);
      formData.append('min_pay_days', document.getElementById("min_pay_days").value);
      formData.append('effective_date', effectiveDate);

      fetch('actions/bonus-rate-master-action.php?action=save_rate', {
        method: 'POST',
        body: formData
      })
      .then(res => res.json())
